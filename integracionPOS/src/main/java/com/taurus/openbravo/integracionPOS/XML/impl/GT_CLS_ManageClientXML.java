...
public class GT_CLS_ManageClientXML extends GT_CLS_Base implements GT_CLS_XMLServiceInterface<GT_CLS_Cliente> {
	...
	public String generarXML(GT_CLS_Cliente cliente, String tipo) {
		if (tipo.equals("location")) {
			...
			String xml = addHeader() + sb.toString() + addFooter();
			return xml;
	...
	public void procesarXML(NodeList nodos) {
		...
		for (int i = 0; i < nodos.getLength(); i++) {
			...
			for (int j = 0; j < nodeList.getLength(); j++) {
				...
				} /* FIN DE PREGUNTAR SI ES EL NODO DE DIRECCION */
			...
			if (buscar("BusinessPartner", "searchKey", cliente.getIdSAP()) == null) {
				...
				if (cliente != null) {
					String xml = generarXML(cliente,
							"location"); /* AQUI GENERA XML PARA C_LOCATION */
	...
	public GT_CLS_Cliente validarDatos(GT_CLS_Cliente cliente) {
		...
		String idPriceList = buscar("PricingPriceList", "name", cliente.getPricelist());// REVISAR
		if (idPriceList != null) {
